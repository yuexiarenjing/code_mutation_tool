...
private:
  ...
  bool isEqChar(SourceLocation Loc) {
    if (!Loc.isValid()) {
      return false;
    }
  ...
  void processDeclStmt(DeclStmt *DS) {
    const Stmt *Parent = AstCtx.getParents(*DS).begin()->get<Stmt>();
    ...
    for (auto *D : DS->decls()) {
      if (auto *VD = dyn_cast<VarDecl>(D)) {
        if (VD->hasInit()) {
          if (hasCallExprChild(VD->getInit())) {
            return;
          }
          ...
          for(int i = -1; i > -255; i --) {
            eqLoc = InitRange.getBegin().getLocWithOffset(i);
            if (isEqChar(eqLoc)) {

              SourceManager &SM = AstCtx.getSourceManager();
              SourceLocation StartLoc = eqLoc;
