...
private:
  ...
  bool isSimpleStmt(Stmt* S) {
    if (isa<BinaryOperator>(S) || isa<UnaryOperator>(S) || isa<CallExpr>(S)) {
      return true;
    }
  ...
  void doFI(std::deque<Stmt*> dStmt) {
    SourceManager &SM = AstCtx.getSourceManager();

    SourceLocation StartLoc = dStmt[0]->getBeginLoc();
    SourceLocation EndLoc = Lexer::findLocationAfterToken(
      dStmt[dStmt.size()-1]->getEndLoc(), tok::semi, SM, AstCtx.getLangOpts(), false);

    addDel(StartLoc, EndLoc);
  }

  void processCompoundStmt(CompoundStmt *CS) {
    // if only BB in compond return
    if (CS->size() == 1) {
      return;
    }

    std::deque<Stmt*> dStmt;
    for (Stmt *S : CS->children()) {
      if (isSimpleStmt(S)) {
        if(dStmt.size() == 5) {
          // do FI
          doFI(dStmt);
