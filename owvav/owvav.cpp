...
private:
  ...
  void doReplace(SourceLocation StartLoc, SourceLocation EndLoc, std::string new_code) {
    ...
    if (StartLoc.isValid() && EndLoc.isValid() 
      ...
      // Rewrite.ReplaceText(RepRange, StringRef(new_code));
      // Rewrite.getEditBuffer(SM.getMainFileID()).write(llvm::outs());
    }
  }

  bool isInForConstruct(Stmt *S) {
  ...
  void processBinaryOperator(BinaryOperator *BO) {
    if (isInForConstruct(BO)) {
      return;
    }

    IntegerLiteral* IL = dyn_cast<IntegerLiteral>(BO->getRHS());
    if (!IL) {
      if (CastExpr* CE = dyn_cast<CastExpr>(BO->getRHS())) {
        for(auto child : CE->children()) {
          IL = dyn_cast<IntegerLiteral>(child);
        }
      }
    }
    ...
    FloatingLiteral* FL = dyn_cast<FloatingLiteral>(BO->getRHS());
    if (!FL) {
      if (CastExpr* CE = dyn_cast<CastExpr>(BO->getRHS())) {
        for(auto child : CE->children()) {
          FL = dyn_cast<FloatingLiteral>(child);
        }
      }
    }
    ...
    if (FL) {
      ...
      DoubleToString(d, s);
      d = StringToDouble(s);
      // uint64_t bits = F.bitcastToAPInt().getZExtValue();
      // uint64_t new_bits = bits ^ 0xFF;
