...
private:
  ...
  void processIfStmt(IfStmt *IS) {
    ...
    if (isBasicBlockOnlyStmt(IS, AstCtx)) {
      return;
    }
    ...
    if (CompoundStmt *CS = dyn_cast<CompoundStmt>(then)) {
      int stmtCount = 0;
    ...
    SourceLocation EndLoc = Lexer::findLocationAfterToken(
      IS->getEndLoc(), tok::semi, SM, AstCtx.getLangOpts(), false);
    if (!EndLoc.isValid()) {
      EndLoc = IS->getEndLoc();
    }
